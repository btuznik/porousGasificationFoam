...
namespace Foam
{
    ...
    defineRunTimeSelectionTable(basicSolidThermo, fvMesh);
}

const Foam::word Foam::basicSolidThermo::dictName("solidThermophysicalProperties");


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

Foam::wordList Foam::basicSolidThermo::heBoundaryBaseTypes()
{
...
Foam::wordList Foam::basicSolidThermo::heBoundaryTypes()
{
    const volScalarField::Boundary& tbf =
        this->T_.boundaryField();
...
Foam::volScalarField& Foam::basicSolidThermo::lookupOrConstruct
(
    const fvMesh& mesh,
    const char* name
) const
...
{
    return New<basicSolidThermo>(mesh, phaseName);
}
...
const Foam::basicSolidThermo& Foam::basicSolidThermo::lookupThermo
(
    const fvPatchScalarField& pf
)
{
    if (pf.db().foundObject<basicSolidThermo>(dictName))
    {
        return pf.db().lookupObject<basicSolidThermo>(dictName);
    }
    else
    {
        HashTable<const basicSolidThermo*> thermos =
            pf.db().lookupClass<basicSolidThermo>();

        for
        (
            HashTable<const basicSolidThermo*>::iterator iter = thermos.begin();
            iter != thermos.end();
            ++iter
        )
        {
...
void Foam::basicSolidThermo::validate
(
    const string& app,
    const word& a
) const
...
Foam::wordList Foam::basicSolidThermo::splitThermoName
(
...
const Foam::volScalarField& Foam::basicSolidThermo::alpha() const
{
    return alpha_;
}


const Foam::scalarField& Foam::basicSolidThermo::alpha(const label patchi) const
{
